(function ($) {
  Drupal.behaviors.farm_area_generate = {
    attach: function (context, settings) {
      ...
      for (var i = 0; i < settings.farm_sensor_loraserver.graphs.length; i++) {

        // Get the graph name, id, and data.
        var name = settings.farm_sensor_loraserver.graphs[i]['name'];
        var id = settings.farm_sensor_loraserver.graphs[i]['id'];
        var data = settings.farm_sensor_loraserver.graphs[i]['data'];
